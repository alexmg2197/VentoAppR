...
export default function VerCorreos(){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire({
            title: "No se pudo eliminar el correo",
            icon: "error",
            draggable: true
          });
        }
  ...
  const newCorreo = () => {
    ...
    return(
        <div className="p-4">
          {
            modalEdit && <ModalEditEmail modal={setModalEdit} correo={correo} isEdit={ismodal}/>
          }
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end' }} className="bg-three">
                <button onClick={()=>{newCorreo()}} className="group mr-3 mb-3 flex items-center justify-start w-11 h-8 bg-green-500 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-30 hover:rounded-lg active:translate-x-1 active:translate-y-1">
            ...
            <TableContainer>
                        ...
                        <TableBody>
                            {currentItems.map((correo, i) => (
                            <TableRow key={correo.idCorreo}>
                                <TableCell>{i+1}</TableCell>
                                {correo.colaborador === null?(
                                  <>
                                    <TableCell colSpan={4} align="center" className="">{"Sin aginar"}</TableCell>
                                  </>
                                ):(
                                  <>
                                    <TableCell>{`${correo.colaborador.nombreColaborador} ${correo.colaborador.apellidoColaborador}`}</TableCell>
                                    <TableCell>{correo.colaborador.puesto}</TableCell>
                                    <TableCell>{correo.colaborador.area.nombreArea}</TableCell>
                                    <TableCell>{correo.colaborador.ubicacion.ubicacion}</TableCell>
                                  </>
                                )}
                                <TableCell>{correo.correo}</TableCell>
                                <TableCell>{correo.contraseñaCorreo}</TableCell>
                                <TableCell>
                                    <div className="flex flex-col items-center justify-center gap-y-1">
                                      <button onClick={() => {editC(correo)}} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-orange-400 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                      ...
                                      {
                                        user.rol != 'Analista' && (
                                          <button onClick={() => { deleteC(correo) }} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                              <div className="flex items-center space-x-1 text-white text-xs font-semibold">
                                ...
                                {/* <button onClick={()=>{editC(correo)}} title="Editar Correo" className='icon-button p-2'><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20"><path fill="currentColor" d="m2.292 13.36l4.523 4.756L.5 20zM12.705 2.412l4.522 4.755L7.266 17.64l-4.523-4.754zM16.142.348l2.976 3.129c.807.848.086 1.613.086 1.613l-1.521 1.6l-4.524-4.757L14.68.334l.02-.019c.119-.112.776-.668 1.443.033"/></svg></button>
                                <button onClick={()=>{deleteC(correo)}} title="Eliminar Correo" className='icon-button'><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 48 48"><defs><mask id="IconifyId19491a687d6412eb80"><g fill="none" strokeLinejoin="round" strokeWidth="4"><path fill="#fff" stroke="#fff" d="M9 10v34h30V10z"/><path stroke="#000" strokeLinecap="round" d="M20 20v13m8-13v13"/><path stroke="#fff" strokeLinecap="round" d="M4 10h40"/><path fill="#fff" stroke="#fff" d="m16 10l3.289-6h9.488L32 10z"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#IconifyId19491a687d6412eb80)"/></svg></button> */}
                                </TableCell>
                            </TableRow>
                            ))}
                        </TableBody>
