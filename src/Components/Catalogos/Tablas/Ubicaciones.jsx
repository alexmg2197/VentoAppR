...
import ModalUbicacion from '../../Modal/ModalUbicacion'

export default function Ubicaciones({search, onComponentListChange, reloadFlag, onRefresh}){
    ...
    const filteredRows = ubicaciones.filter(item =>
    ...
    const deleteU = (datos) => {
        Swal.fire({
            title: "¿Estas seguro que deseas eliminar la ubicacion?",
            ...
            try {
                await axios.patch(`${API_URL}/api/Ubicaciones/EliminarUbicacion/${datos.idUbicacion}`)
    ...
    return(
            <div className="p-4">
                    {
                    modalEdit && <ModalUbicacion modal={setModalEdit} ubicacion={ubicacion} isEdit={ismodal} onRefresh={onRefresh}/>
                    }
                    <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
                        <button onClick={()=>{newUbi()}} className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-35 hover:rounded-lg active:translate-x-1 active:translate-y-1">
                            ...
                            <TableRow key={ubi.idUbicacion}>
                                ...
                                <TableCell>
                                    <div className="flex items-center justify-center gap-x-2">
                                        <button onClick={() => {editU(ubi)}} className="group relative  flex items-center justify-start w-fit px-3 py-1 bg-yellow-500 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                        ...
                                        <button onClick={() => { deleteU(ubi) }} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                            <div className="flex items-center space-x-1 text-white text-xs font-semibold">
