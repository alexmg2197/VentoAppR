...
import ModalSistemaOperativo from "../../Modal/ModalSistemaOperativo";

export default function SistemaOperativo({search, onComponentListChange, reloadFlag, onRefresh}){
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/api/SistemaOperativo`)
            ...
            .then((data) => {setLoading(false); setSistemas(data)});
    }, [reloadFlag]);
    ...
    const filteredRows = sistemas.filter(item =>
        item.sistemasOperativo.toLowerCase().includes(search.toLowerCase()) 
    );
    ...
    const deleteS = (datos) => {
        Swal.fire({
            title: "¿Estas seguro que deseas eliminar el sistema operativo?",
            ...
            try {
                await axios.patch(`${API_URL}/api/SistemaOperativo/EliminarSistemaOperativo/${datos.idSistemaOperativo}`)
    
                await Swal.fire({
                    title: "Eliminado",
                    text: "Sistema Operativo ha sido eliminada.",
                    icon: "success"
                    }).then(async(result)=>{
                    if(result.isConfirmed){
                        await onRefresh()
                ...
                Swal.fire({
                title: "No se pudo eliminar el Sistema Operativo",
                icon: "error",
                draggable: true
                });
            }
    ...
    return(
            <div className="p-4">
                    {
                    modalEdit && <ModalSistemaOperativo modal={setModalEdit} sistema={sistema} isEdit={ismodal} onRefresh={onRefresh}/>
                    }
                    <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
                        <button onClick={()=>{newSis()}} className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-55 hover:rounded-lg active:translate-x-1 active:translate-y-1">
                    ...
                    <TableContainer>
                        ...
                        <TableBody>
                            {paginatedData.map((sistema, i) => (
                            <TableRow key={sistema.idSistemaOperativo}>
                                <TableCell>{i+1}</TableCell>
                                <TableCell>{`${sistema.sistemasOperativo}`}</TableCell>
                                <TableCell>
                                    <div className="flex items-center justify-center gap-x-2">
                                        <button onClick={() => {editS(sistema)}} className="group relative  flex items-center justify-start w-fit px-3 py-1 bg-yellow-500 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                        ...
                                        <button onClick={() => { deleteS(sistema) }} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                            <div className="flex items-center space-x-1 text-white text-xs font-semibold">
