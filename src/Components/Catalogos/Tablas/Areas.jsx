...
export default function Areas({search, onComponentListChange, reloadFlag, onRefresh }){
    ...
    // Función para aplicar el filtro y devolver las filas filtradas
        const filteredRows = areas.filter(item =>
        item.nombreArea.toLowerCase().includes(search.toLowerCase()) ||
        item.responsableArea.toLowerCase().includes(search.toLowerCase()) 
    );
    ...
    const deleteA = (datos) => {
        Swal.fire({
            ...
            try {
                await axios.patch(`${API_URL}/api/Areas/EliminarArea/${datos.idArea}`)
                ...
                Swal.fire({
                title: "No se pudo eliminar el area",
                icon: "error",
                draggable: true
                });
            }
    ...
    return(
            <div className="p-4">
                    {
                    modalEdit && <ModalAddArea modal={setModalEdit} area={area} isEdit={ismodal} onRefresh={onRefresh}/>
                    }
                    <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
                        <button onClick={()=>{newArea()}} className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-25 hover:rounded-lg active:translate-x-1 active:translate-y-1">
                        ...
                        <TableBody>
                            ...
                            <TableRow key={area.idArea}>
                                ...
                                <div className="flex items-center justify-center gap-x-2">
                                    <button onClick={() => {editA(area)}} className="group relative  flex items-center justify-start w-fit px-3 py-1 bg-yellow-500 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                    ...
                                    <button onClick={() => { deleteA(area) }} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                        <div className="flex items-center space-x-1 text-white text-xs font-semibold">
